...
1. **Latency.** It processes audio in short chunks (default ~0.6s, plus
...
2. **CPU use.** It runs on your CPU (no GPU required). A modern
...
1. Do steps 1–2 below (install Python 3.10/3.11 and VB-CABLE).
2. Double-click `build_exe.bat` in this folder and follow the prompts.
3. When it finishes, `dist\VoiceIsolator.exe` is a real standalone app.
...
1. Create a free GitHub account if you don't have one, and push this
   ...
   you once).
2. In the repo, go to the **Actions** tab → **Build Windows exe** →
   **Run workflow**.
3. Wait a few minutes, then open the finished run and download the
...
1. Go to https://vb-audio.com/Cable/ and download **VB-CABLE A+B**.
2. Run the installer as Administrator, reboot if it asks.
3. You'll now have 4 new devices in Windows: `CABLE-A Input`,
